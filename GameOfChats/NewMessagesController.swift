...
//        cell.imageView?.image = UIImage(named: "Bill-Gates")
        ...
        if let profileImageUrl = user.profileImageUrl {
           cell.profileImageView.loadProfileImagesInCacheWithURL(urlString: profileImageUrl)
        }
        return cell
    }
    ...
    var myOwnMessgesController: MyOwnMessagesController?
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
       ...
       dismiss(animated: true) {
        let user = self.users[indexPath.item]
        self.myOwnMessgesController?.showChatLogForUser(user: user)
